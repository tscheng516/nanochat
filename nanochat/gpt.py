...
class GPT(nn.Module):
    def __init__(self, config, pad_vocab_size_to=64):
        ...
        self.backout_lambda = nn.Parameter(0.2 * torch.ones(1) if config.reinit else 0.2 * torch.ones(1))
    ...
    def init_weights(self):
        ...
        for i in range(n_layer):
            self.x0_lambdas.data[i] = (
                0.5 * n_layer * (0.2 / n_layer) ** (i / max(n_layer - 1, 1))
                if self.config.reinit
    ...
    def setup_optimizer(self, unembedding_lr=0.008, embedding_lr=0.3, matrix_lr=0.02, weight_decay=0.28, scalar_lr=0.5):
        ...
        dmodel_lr_scale = (model_dim / 768) ** -0.5 # =0.707 for d=24
        ...
        if self.config.reoptim:
            ...
            param_groups = [
                ...
                dict(kind='adamw', params=smear_params, lr=0.2, betas=(0.8, 0.95), eps=1e-10, weight_decay=0.0), # backout split off
                dict(kind='adamw', params=backout_params, lr=0.2, betas=(0.8, 0.95), eps=1e-10, weight_decay=0.0), # identical to smear params
            ]
